...
import { getUserDashboardData, updateLastOpenedWebsite, type RecentArticle, type RecentKeyword, type UserDashboardData } from "@/lib/api/user-dashboard.api";
...
export default function UserDashboard() {
  ...
  const websiteId = params.websiteId?.toString() || "";

  useEffect(() => {
    const updateUserInfo = async () => {
      try {
        await updateLastOpenedWebsite(websiteId);
      } catch (err) {
        console.error("Failed to update last opened website:", err);
      }
    };
  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      try {
        setLoading(true);
        setError(null);
        const data = await getUserDashboardData(websiteId);
